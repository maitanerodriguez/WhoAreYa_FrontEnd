...
export function setupRows(game) {
    ...
    function showContent(content, guess) {
        ...
        for (let j = 0; j < content.length; j++) {
            ...
            let emaitza = check(attribs[j], guess[attribs[j]]);
            let gezia = "";   // hemen jarriko dugu gezi ikonoa

            if (attribs[j] === "birthdate") {

                if (emaitza === "higher") {
                    // jokalari misteriotsua ZAHARRAGO da → goranzko gezia
                    gezia = higher;
                } else if (emaitza === "lower") {
                    // jokalari misteriotsua GAZTEAGO da → beherako gezia
                    gezia = lower;
                }
            }
